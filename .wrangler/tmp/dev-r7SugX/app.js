...
(() => {
  ...
  // .wrangler/tmp/bundle-lKZ60q/checked-fetch.js
...
`
  ...
  // C:/Users/57300/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/loader-sw.ts
  ...
  globalThis.dispatchEvent = function(event2) {
    if (__facade_isSpecialEvent__(event2.type)) {
      return __FACADE_EVENT_TARGET__.dispatchEvent(event2);
    } else {
      return __facade__originalDispatchEvent__(event2);
    }
  };
  ...
  __facade__originalAddEventListener__("fetch", (event2) => {
    const ctx = {
      waitUntil: event2.waitUntil.bind(event2),
      passThroughOnException: event2.passThroughOnException.bind(event2)
    };
    const __facade_sw_dispatch__ = /* @__PURE__ */ __name(function(type, init) {
      if (type === "scheduled") {
        ...
        event2.waitUntil(Promise.all(facadeEvent[__facade_waitUntil__]));
      }
  ...
  __facade__originalAddEventListener__("scheduled", (event2) => {
    const facadeEvent = new __Facade_ScheduledEvent__("scheduled", {
      scheduledTime: event2.scheduledTime,
      cron: event2.cron,
      noRetry: event2.noRetry.bind(event2)
    });
    __FACADE_EVENT_TARGET__.dispatchEvent(facadeEvent);
    event2.waitUntil(Promise.all(facadeEvent[__facade_waitUntil__]));
  });
  ...
  // .wrangler/tmp/bundle-lKZ60q/middleware-insertion-facade.js
  ...
  async function cargarProximoNumeroOrden() {
    try {
      ...
      console.error("Error al cargar n\xFAmero de orden:", error);
    }
  }
  __name(cargarProximoNumeroOrden, "cargarProximoNumeroOrden");
  async function cargarTecnicosParaFiltro() {
    try {
      ...
      if (data.success && data.tecnicos) {
        ...
        select.innerHTML = '<option value="">Todos los t\xE9cnicos</option>';
      ...
      console.error("Error al cargar t\xE9cnicos para filtro:", error);
    }
  }
  __name(cargarTecnicosParaFiltro, "cargarTecnicosParaFiltro");
  document.addEventListener("DOMContentLoaded", function() {
