  ...
  try {
      ...
      return new Response(`
        ...
        <body class="bg-gray-100 flex items-center justify-center min-h-screen p-4">
          <div class="bg-white rounded-xl shadow-lg p-8 text-center max-w-md">
            ...
            <p class="text-sm text-gray-500 mt-4">Token: ${token}</p>
            <p class="text-sm text-gray-500">Contacte al taller para más información.</p>
          </div>
        </body>
        </html>
      `, {
    ...
    return new Response(`
      ...
      <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Error - Global Pro</title>
        <script src="https://cdn.tailwindcss.com"></script>
      </head>
      <body class="bg-red-100 flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-xl shadow-lg p-8 text-center max-w-md">
          <div class="text-6xl mb-4">⚠️</div>
          <h1 class="text-2xl font-bold text-red-800 mb-2">Error del Servidor</h1>
          <p class="text-gray-600">Ha ocurrido un error al cargar la orden.</p>
          <p class="text-sm text-gray-500 mt-4">Token: ${token}</p>
...
<body class="p-4">
    ...
    <script>
        ...
        async function aprobarOrden() {
            ...
            try {
                console.log('Enviando firma al servidor...');
                console.log('Token:', TOKEN);
                console.log('Longitud de firma:', imageData.length);
                ...
                alert('Error de conexión. Intente nuevamente. Error: ' + error.message);
                btn.innerHTML = '✅ Aceptar y Firmar';
                btn.disabled = false;
            }
        }
                ...
                <body class="bg-green-100 flex items-center justify-center min-h-screen p-4">
                    <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center">
                        ...
                        <div class="bg-green-50 rounded-xl p-4 mb-6">
                            ...
                            <p class="text-sm text-gray-600 mt-2">Patente: <strong>${orden.patente_placa}</strong></p>
                        </div>
                        ...
                        <button onclick="cerrarPagina()" class="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-4 px-6 rounded-xl mb-3 transition">
                            ✅ Finalizar
                        </button>

                        <p class="text-sm text-gray-500 mt-4">¡Gracias por confiar en Global Pro Automotriz!</p>
                    </div>

                    <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"><\/script>
                    <script>
                        function cerrarPagina() {
                            // Intentar cerrar la ventana o pestaña
                            try {
                                window.close();
                            } catch(e) {
                            ...
                            setTimeout(() => {
                                alert('Puede cerrar esta ventana ahora.');
                            }, 100);
                        }
                    <\/script>
                </body>
