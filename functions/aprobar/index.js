...
function getApprovalPage(orden, token) {
  ...
  html += 'var ctx = canvas ? canvas.getContext("2d") : null;';
  html += 'var drawing = false;';
  html += 'var TOKEN = "' + token + '";';
  html += 'function resizeCanvas() {';
  html += '  if (!canvas || !ctx) return;';
  html += '  var container = canvas.parentElement || canvas;';
  html += '  var width = Math.max(300, container.clientWidth);';
  html += '  canvas.style.width = "100%";';
  html += '  canvas.style.height = "250px";';
  html += '  canvas.width = width;';
  html += '  canvas.height = 250;';
  html += '  ctx.lineWidth = 4;';
  html += '  ctx.lineCap = "round";';
  html += '  ctx.strokeStyle = "#000000";';
  html += '  canvas.style.touchAction = "none";';
  html += '}';
  html += 'function getPos(e) {';
  html += '  var rect = canvas.getBoundingClientRect();';
  html += '  var clientX = e.clientX || (e.touches && e.touches[0] && e.touches[0].clientX) || 0;';
  html += '  var clientY = e.clientY || (e.touches && e.touches[0] && e.touches[0].clientY) || 0;';
  ...
  html += '  ctx.stroke();';
  html += '}';
  html += 'function endDraw(e) {';
  html += '  if (!ctx) return;';
  html += '  drawing = false;';
  html += '  ctx.closePath();';
  html += '  if (e && e.pointerId && canvas.releasePointerCapture) canvas.releasePointerCapture(e.pointerId);';
  html += '}';
  html += 'window.addEventListener("load", resizeCanvas);';
  html += 'window.addEventListener("resize", resizeCanvas);';
  html += 'if (window.PointerEvent) {';
  html += '  canvas.addEventListener("pointerdown", function(e) { e.preventDefault(); if (canvas.setPointerCapture) canvas.setPointerCapture(e.pointerId); startDraw(e); });';
  html += '  canvas.addEventListener("pointermove", moveDraw);';
  html += '  canvas.addEventListener("pointerup", endDraw);';
  html += '  canvas.addEventListener("pointerleave", endDraw);';
  html += '  canvas.addEventListener("pointercancel", endDraw);';
  html += '} else {';
  html += '  canvas.addEventListener("mousedown", function(e) { e.preventDefault(); startDraw(e); });';
  ...
  html += '  canvas.addEventListener("touchstart", function(e) { e.preventDefault(); startDraw(e); }, { passive: false });';
  html += '  canvas.addEventListener("touchmove", function(e) { e.preventDefault(); moveDraw(e); }, { passive: false });';
