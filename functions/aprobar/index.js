...
function getApprovedPage(orden) {
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Orden Aprobada</title><script src="https://cdn.tailwindcss.com"></script><script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"></script></head><body class="bg-green-100 flex items-center justify-center min-h-screen p-4"><div class="bg-white rounded-2xl shadow-2xl p-8 text-center max-w-md"><div class="text-6xl mb-4">✅</div><h1 class="text-3xl font-black text-green-700 mb-2">¡Orden Aprobada!</h1><p class="text-gray-600 mb-4">Su firma ha sido guardada exitosamente.</p><div class="bg-green-50 rounded-xl p-4 mb-6"><p class="text-sm text-gray-600">Orden N°</p><p class="text-2xl font-bold text-green-700">' + n + '</p><p class="text-sm text-gray-500 mt-2">Fecha: ' + (orden.fecha_aprobacion || 'N/A') + '</p></div>' + firmaImg + '<div class="mt-6 space-y-3"><button onclick="descargarPDF()" class="w-full bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 transition">📄 Descargar PDF</button><button onclick="verPDFEnLinea()" class="w-full bg-green-600 text-white py-2 px-4 rounded-lg hover:bg-green-700 transition">👁️ Ver en Línea</button></div><p class="text-sm text-gray-500 mt-6">¡Gracias por confiar en Global Pro Automotriz!</p></div><script>const ordenData = ' + JSON.stringify(orden) + '; function descargarPDF() { const { jsPDF } = window.jspdf; const doc = new jsPDF("p", "mm", "a4"); const numeroFormateado = String(ordenData.numero_orden).padStart(6, "0"); doc.setFontSize(20); doc.text("ORDEN DE TRABAJO", 105, 20, { align: "center" }); doc.setFontSize(16); doc.text("Global Pro Automotriz", 105, 30, { align: "center" }); doc.setFontSize(12); doc.text("N° " + numeroFormateado, 20, 50); doc.text("Cliente: ' + cliente + '", 20, 60); doc.text("Patente: ' + patente + '", 20, 70); doc.text("Vehículo: ' + marca + ' ' + modelo + '", 20, 80); doc.text("Trabajos: ' + trabajosStr + '", 20, 90); doc.text("Total: $' + total + '", 20, 100); doc.text("Abono: $' + abono + '", 20, 110); doc.text("Restante: $' + restante + '", 20, 120); doc.save("OT-" + numeroFormateado + "-" + (ordenData.patente_placa || "N/A") + ".pdf"); } function verPDFEnLinea() { const { jsPDF } = window.jspdf; const doc = new jsPDF("p", "mm", "a4"); const numeroFormateado = String(ordenData.numero_orden).padStart(6, "0"); doc.setFontSize(20); doc.text("ORDEN DE TRABAJO", 105, 20, { align: "center" }); doc.setFontSize(16); doc.text("Global Pro Automotriz", 105, 30, { align: "center" }); doc.setFontSize(12); doc.text("N° " + numeroFormateado, 20, 50); doc.text("Cliente: ' + cliente + '", 20, 60); doc.text("Patente: ' + patente + '", 20, 70); doc.text("Vehículo: ' + marca + ' ' + modelo + '", 20, 80); doc.text("Trabajos: ' + trabajosStr + '", 20, 90); doc.text("Total: $' + total + '", 20, 100); doc.text("Abono: $' + abono + '", 20, 110); doc.text("Restante: $' + restante + '", 20, 120); const pdfBlob = doc.output("blob"); const pdfUrl = URL.createObjectURL(pdfBlob); window.open(pdfUrl, "_blank"); }</script></body></html>';
}
